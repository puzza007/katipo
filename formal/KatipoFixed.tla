...
(* here.                                                                   *)
(*                                                                         *)
(* Admission-capacity rejection (max_in_flight) is not modeled: a full     *)
(* worker rejects the admission call synchronously before the request      *)
(* enters Reqs -- a strict restriction of SendAsync/WorkerRecvReq that     *)
(* adds no delivery paths, so the properties are preserved by              *)
(* construction (a rejected request simply never exists in the model).    *)
(*                                                                         *)
