...
(* here.                                                                   *)
(*                                                                         *)
(* Admission-capacity rejection (max_in_flight) and the bounded admission  *)
(* call (admission_timeout) are not modeled: both reject synchronously     *)
(* before the request enters Reqs -- strict restrictions of                *)
(* SendAsync/WorkerRecvReq that add no delivery paths, so the properties   *)
(* are preserved by construction (a rejected request never exists in the   *)
(* model, and a late-admitted one resolves via its own timer as already    *)
(* modeled).                                                               *)
(*                                                                         *)
